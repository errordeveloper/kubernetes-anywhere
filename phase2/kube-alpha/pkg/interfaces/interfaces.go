...
type FirstMasterDiscoveryProvider interface {
	Setup() // install and/or configure
	Launch()
}

type AnyMasterDiscoveryProvider interface {
	Setup()
	Launch()
}

type NodeDiscoveryProvider interface {
	Setup()
	Launch()
}
...
type FirstMasterLauncher interface {
	Launch()
}

type AnyMasterLauncher interface {
	Launch()
}
